...
// matlab API: [V] = crbmReconCUDA(H, W, vb, pars);
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{  
    float *H = (float*)mxGetData(prhs[0]);
    ...
    plhs[0] = mxCreateNumericMatrix(size_V_batch, 1, mxSINGLE_CLASS, mxREAL);
    float *V  = (float*)mxGetData(plhs[0]);

	init_const(V,  0, size_V_batch);

	crbmRecon_proxy(V, H, W, vb, pars);
}
